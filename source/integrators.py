...
class ElementIntegratorGalerkin:
    ...
    def classify_integration_type(self,
        ...
        Args:
            x_centroid (np.ndarray): Centroid of observation element, shape 
                (3,).
            y_centroids (np.ndarray): Centroids of source elements, shape
                (K, 3).
            y_sizes (np.ndarray): Characteristic sizes of source elements,
                shape (K,).
            x_elem_idx (Optional[int]): Field element index (for singular
                integrals).
            y_elem_indices (Optional[np.ndarray]): Indices of source elements,
                shape (K,).

        Returns:
            np.ndarray: Array of strings indicating integration type for each
                interaction, shape (K,).
                Values are 'singular', 'near_singular', or 'regular'.
        """
        K = len(y_centroids)
